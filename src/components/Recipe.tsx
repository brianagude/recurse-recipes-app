...
export function Recipe({ recipe }: { recipe: RecipeType }) {
  ...
  return (
    <>
    <main className={`min-h-screen h-screen ${recipe.blog_post ? 'flex flex-row-reverse' : ''}`}>
      {recipe.blog_post && 
        <div className="p-6 space-y-6 basis-1/2 overflow-y-auto">
          <p>{recipe.blog_post}</p>
        </div>
      }
      <div className={`p-6 space-y-6 ${recipe.blog_post ? 'basis-1/2 overflow-y-auto' : 'w-full max-w-5xl mx-auto'}`}>
        <div>
